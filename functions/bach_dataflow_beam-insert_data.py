...
class IngestDoFn(beam.DoFn):
    ...
    def process(self, element):
        ...
        bucket_name = 'data-scraper-imobiliaria'
...
data_transform = (
    p_transform
    | "Leitura dos dados" >> beam.io.ReadFromText(r'gs://data-scraper-imobiliaria/data_scraper_imobiliario.csv', skip_header_lines=1)
...
#   | "Salvando as taxas em uma nova coluna 6" >> beam.Map(lambda cols: [cols[i] for i in range(len(cols))] + [cols[5].split()[1]] if len(cols[5].split()) > 1 else [cols[i] for i in range(len(cols))] + [np.nan])
    ...
    | "Print Transformed Rows" >> beam.io.WriteToText(r'gs://data-imobiliario/data_imobiliario_tratado.json')
