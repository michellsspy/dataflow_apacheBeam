...
pipeline_options = {
    ...
    'region': 'us-east1',
    'staging_location': 'analise-imobiliario-bc/staging-location',
    'temp_location': 'analise-imobiliario-bc/temp',
    'template_location': 'analise-imobiliario-bc/template-analise-imobiliario/batch_job_insert_transform.json'
}
...
class IngestDoFn(beam.DoFn):
    ...
    def process(self, element):
        ...
        # Obtendo o bucket do Google Cloud Storage
        bucket = storage_client.bucket(bucket_name)

        # Número de iterações antes de fazer o upload para o GCS
        upload_frequency = 10
        upload_counter = 0
        ...
        with open(csv_file, "w", newline='', encoding='utf-8') as file:
            ...
            for i in range(160):
                try:
                    ...
                    # Atualizar o contador de upload
                    upload_counter += 1

                    # Upload do arquivo CSV para o Google Cloud Storage após um certo número de iterações
                    if upload_counter >= upload_frequency:
                        blob = bucket.blob(bronze_folder + '/' + csv_file)
                        blob.upload_from_filename(csv_file)
                        # Resetar o contador de upload
                        upload_counter = 0
...
#   | "Salvando as taxas em uma nova coluna 6" >> beam.Map(lambda cols: [cols[i] for i in range(len(cols))] + [cols[5].split()[1]] if len(cols[5].split()) > 1 else [cols[i] for i in range(len(cols))] + [np.nan])
    ...
    | "Print Transformed Rows" >> beam.io.WriteToText(f'gs://{bucket_name}/{silver_folder}/{arquive_name_silver}')
