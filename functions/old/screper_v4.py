...
with open(csv_file, "w", newline='', encoding='utf-8') as file:
    ...
    writer.writerow(['Título', 'Nome da Rua', 'Numero', 'Bairro', 'Cidade', 'Preço', 'Área', 'Quartos', 'Banheiros', 'Garagens'])
    ...
    for i in range(20):
        ...
        for span in spans:
            ...
            # Extrair o bairro e o número
            if bairro:
                bairro_split = bairro.split()
                if len(bairro_split) > 1 and bairro_split[0].isdigit():
                    numero = int(bairro_split[0])
                    bairro = ' '.join(bairro_split[1:])
                else:
                    numero = None
